        ...
        if event_type == "conversation.created":
            ...
            if consultation:
                ...
                if "assignee" in conversation:
                    ...
                    # BUG FIX #1: НЕ отправляем уведомления для закрытых консультаций
                    terminal_statuses = {"closed", "resolved", "cancelled"}
                    current_status = consultation.status
                    is_terminal = current_status in terminal_statuses
                    
                    # Если менеджер изменился И консультация НЕ закрыта, отправляем уведомление
                    if manager_changed and not is_terminal:
                        try:
            ...
            # BUG FIX #6: Обрабатываем новые сообщения от клиентов
            ...
            if message_type == "incoming" and sender_type == "contact":
                ...
                if consultation:
                    # Обновляем updated_at консультации
                    ...
                    try:
                        from ..routers.websocket import notify_consultation_update
                        await notify_consultation_update(conversation_id, consultation)
                    except Exception as ws_error:
                        logger.debug(f"Failed to notify WebSocket clients: {ws_error}")
            else:
                # Исходящие сообщения (от агентов) не обрабатываем здесь
                logger.debug(f"Skipping message: type={message_type}, sender={sender_type}")
            ...
            if consultation:
                ...
                # ═══════════════════════════════════════════════════════════════════════
                # GUARD CLAUSE: Терминальные статусы НЕ МЕНЯЕМ обратно
                # ═══════════════════════════════════════════════════════════════════════
                terminal_statuses = {"closed", "resolved", "cancelled"}
                
                # Если консультация в терминальном статусе и новый статус не терминальный - пропускаем
                if old_status in terminal_statuses and new_status not in ("resolved", "closed"):
                    logger.info(
                        f"Status update skipped (toggle_status): consultation {cons_id} has terminal status '{old_status}', "
        ...
        if event_type == "consultation.created":
            ...
            if consultation:
                ...
                if "status" in event_data:
                    ...
                    # Защита терминальных статусов
                    if old_status in terminal_statuses and new_status not in ("resolved", "closed"):
                        logger.info(
                            f"Status update skipped (1C event): consultation has terminal status '{old_status}', "
                            f"not updating to '{new_status}'"
                        )
                    else:
                        consultation.status = new_status
            ...
            logger.info(f"Processing consultation.closed: cons_id={cons_id}, cl_ref_key={cl_ref_key}")
            
            consultation = None
            
            # Try to find by cons_id first
            if cons_id:
                result = await db.execute(
                    select(Consultation).where(Consultation.cons_id == cons_id)
                )
            ...
            # If not found by cons_id, try by cl_ref_key
            if not consultation and cl_ref_key:
                result = await db.execute(
                    select(Consultation).where(Consultation.cl_ref_key == cl_ref_key)
                )
                consultation = result.scalar_one_or_none()
            ...
            if not consultation:
                logger.warning(f"Consultation not found for closing: cons_id={cons_id}, cl_ref_key={cl_ref_key}")

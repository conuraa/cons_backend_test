...
async def verify_api_token(
    api_token: Optional[str] = Query(default=None, description="API token for authentication"),
) -> None:
    """
    Проверяет токен из query параметра для Dashboard.
    
    Используется для Dashboard Apps в Chatwoot (iframe),
    где нельзя передавать заголовки.
    
    ВАЖНО: Использует ОТДЕЛЬНЫЙ токен DASHBOARD_API_TOKEN,
    который НЕ даёт доступ к другим API методам.
    Это безопасно для публичного использования в URL.
    
    Пример: /api/managers/dashboard?api_token=YOUR_DASHBOARD_TOKEN
    """
    dashboard_token = settings.DASHBOARD_API_TOKEN
    
    if not dashboard_token:
        # Если токен не задан, пропускаем проверку (dev окружение)
        return
    
    if _safe_equals(dashboard_token, api_token):
        return

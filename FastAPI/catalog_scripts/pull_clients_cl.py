...
async def upsert_client(db: AsyncSession, item: Dict[str, Any]) -> Tuple[bool, bool]:
    ...
    if existing_client:
        ...
        if email and existing_client.email != email:
            ...
            updated = True
        # ВАЖНО: Защита от перезаписи телефона, обновлённого через фронтенд.
        # Если клиент был обновлён в БД менее 10 минут назад и телефон отличается,
        # значит свежие данные были установлены через фронтенд и ещё не долетели в 1С.
        if phone and existing_client.phone_number != phone:
            client_updated_at = existing_client.updated_at
            if client_updated_at and client_updated_at.tzinfo is None:
                client_updated_at = client_updated_at.replace(tzinfo=timezone.utc)
            recently_updated = (
                client_updated_at
                and (datetime.now(timezone.utc) - client_updated_at).total_seconds() < 600
            )
            if recently_updated and existing_client.phone_number:
                logger.info(
                    f"Skipping phone update for client {existing_client.client_id}: "
                    f"DB phone='{existing_client.phone_number}' differs from 1C phone='{phone}', "
                    f"client was updated {(datetime.now(timezone.utc) - client_updated_at).total_seconds():.0f}s ago (< 600s)"
                )
            else:

...
class Settings(BaseSettings):
    ...
    CHATWOOT_AGENT_BOT_TOKEN: Optional[str] = None  # Токен Agent Bot для отправки автосообщений (не влияет на SLA)
    ...
    # Site (фронтенд)
    SITE_BASE_URL: Optional[str] = Field(default=None, description="Базовый URL сайта (фронтенд), например https://dev.clobus.uz. Используется для генерации динамических ссылок.")

    # Anti-Spam Settings (для фильтрации спама через виджет Chatwoot)
    SPAM_FILTER_ENABLED: bool = Field(default=True, description="Включить антиспам-фильтр")
    SPAM_MAX_CONVERSATIONS_PER_HOUR: int = Field(default=5, description="Макс. кол-во обращений от одного контакта в час")
    SPAM_MIN_MESSAGE_LENGTH: int = Field(default=3, description="Минимальная длина первого сообщения")
    SPAM_MAX_LINKS_IN_FIRST_MESSAGE: int = Field(default=2, description="Макс. кол-во ссылок в первом сообщении")
    SPAM_BLOCKED_PHRASES: str = Field(default="", description="Заблокированные фразы через запятую")
    SPAM_AUTO_CLOSE: bool = Field(default=True, description="Автоматически закрывать спам-обращения")
    SPAM_LABEL: str = Field(default="spam", description="Метка для спам-обращений в Chatwoot")

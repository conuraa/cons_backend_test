...
class ChatwootClient:
    ...
    async def _request(
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 429:
                ...
                logger.warning(
                    f"Rate limited (429) on {method} {endpoint}. Waiting {wait_time}s before retry."
                )
                await asyncio.sleep(wait_time)
                # Recursive retry with decremented counter
                return await self._request(method, endpoint, data, params, max_retries - 1) if max_retries > 1 else {}
    ...
    async def _request_public_api(
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 429:
                ...
                wait_time = min(retry_after, 30)
                logger.warning(
                    f"Rate limited (429) on Public API {method} {endpoint}. Waiting {wait_time}s before retry."
                )
                await asyncio.sleep(wait_time)
                return await self._request_public_api(method, endpoint, data, params, max_retries - 1) if max_retries > 1 else {}
    ...
    async def send_activity_message(
        ...
        """
        Отправка activity сообщения (системного уведомления).
        
        Activity сообщения:
        - Видны клиенту в чате
        - НЕ влияют на SLA (First Response Time)
        - Отображаются как системные записи
        
        Используется для автоуведомлений (очередь, переназначение менеджера).
        ...
        payload = {
            "content": content,
            "message_type": 2,  # 0=incoming, 1=outgoing, 2=activity
            "private": False
        }
        return await self._request(
            "POST",
    ...
    async def send_bot_message(
        ...
        """
        Отправка сообщения через Agent Bot API.
        
        Agent Bot сообщения:
        - Видны клиенту в чате (message_type: outgoing)
        - НЕ влияют на SLA (First Response Time)
        - Отображаются как сообщения от бота
        
        Используется для автоуведомлений (принятие заявки, очередь, переназначение).
        """
        bot_token = settings.CHATWOOT_AGENT_BOT_TOKEN
        
        if not bot_token:
            # Fallback на activity message если нет токена бота
            logger.warning("CHATWOOT_AGENT_BOT_TOKEN not set, falling back to activity message")
            return await self.send_activity_message(conversation_id, content)
        ...
        # Используем токен Agent Bot вместо обычного API токена
        async with httpx.AsyncClient(timeout=30.0) as client:

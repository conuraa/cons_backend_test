...
from telegram import Bot, Update, InlineKeyboardButton, InlineKeyboardMarkup, WebAppInfo, MenuButtonWebApp, ReplyKeyboardMarkup, KeyboardButton, ReplyKeyboardRemove
from telegram.ext import Application, CommandHandler, MessageHandler, CallbackQueryHandler, ContextTypes, filters  # BUG FIX #9
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                ...
                if not is_open:
                    ...
                    await update.message.reply_text(
                        f"Диалог завершён.\n\n"
                        f"Эта консультация {status_text}.\n\n"
                        f"Если возникнут вопросы — мы всегда на связи! "
                        f"Создайте новую заявку через портал поддержки."
                    )
                    return
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                ...
                if consultation.status in ("closed", "resolved", "cancelled"):
                    ...
                    # Очищаем контекст пользователя, чтобы не использовать закрытую консультацию
                    if context.user_data:
                        context.user_data.pop("active_cons_id", None)
                        context.user_data.pop("active_client_id", None)
